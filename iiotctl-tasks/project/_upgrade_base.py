...
def upgrade(set_up_tooling: bool, render_manifests: bool, render_readme: bool):
    ...
    if has_merge_conflict:
        print_error("First solve all merge conflicts before starting upgrade process.")
        raise typer.Abort()
    
    if not Command.check_output(["git", "status", "-s"]):
        print_error("No file changes found to upgrade repo with.")
        raise typer.Abort()

    update_branch = f"update/base-{BASE_REPO_VERSION}"

    if update_branch in Command.check_output(["git", "branch"]):
        print_error(f"Update branch '{update_branch}' already exists.")
        raise typer.Abort()
