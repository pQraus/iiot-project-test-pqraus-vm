...
def rollout_restart_deployment(deployment: str, namespace: str, **kwargs):
    Command.check_output(
        cmd=[
            "kubectl",
            "-n",
...
def apply(file: str, **kwargs):
    Command.check_output(cmd=["kubectl", "apply", file, *parse_kwargs_to_cli_args(**kwargs)])
